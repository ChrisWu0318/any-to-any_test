...
def create_annotation_text(data_dir, annotation_path):
    ...
    # If the add punctuation feature is enabled, then import the punctuation recovery model
    if args.add_pun:
        import logging
    ...
    # If the annotation path does not exist, then create it.
    if not os.path.exists(annotation_path):
        os.makedirs(annotation_path)

    # Open the training and testing output files
    ...
    # Read the original transcript text
    transcript_path = os.path.join(data_dir, 'transcript', 'aishell_transcript_v0.8.txt')
    transcript_dict = {} # Used for storing the mapping of audio names to text
    with open(transcript_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()

    # Build an audio ID to text mapping
    ...
    # Annotation for constructing training and development sets
    ...
    for type in data_types:
        ...
        for subfolder, _, filelist in sorted(os.walk(audio_dir)):
            for fname in filelist:
                ...
                lines.append(line)
    # # Calculate the duration of each audio and supplement the structural information
    ...
    # Add audio duration information to the test set
    for i in tqdm(range(len(lines))):
        audio_path = lines[i]['audio']['path']
        sample, sr = soundfile.read(audio_path)
        duration = round(sample.shape[-1] / float(sr), 2)
